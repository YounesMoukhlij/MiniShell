...
/*   Updated: 2024/03/15 13:55:49 by youmoukh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    mini->pipex.path = malloc(sizeof(char) * ft_strlen(ft_get_path(mini)) + 1);
    if (!mini->pipex.path)
        return (free (mini->pipex.path));
    mini->pipex.path = ft_get_path(mini);
    mini->pipex.path_d = ft_split_executor(ft_get_path(mini), ':');
}
